...
const PopupContainer: React.FC<Props> = ({ server, create, resolve }) => {
    ...
    const addServer = useMCPSlice((state) => state.addServer)
    const updateServer = useMCPSlice((state) => state.updateServer)
    ...
    const onOK = async () => {
        ...
        try {
            ...
            if (server && !create) {
                ...
                if (window.electron && window.myAPI?.mcp?.updateServer) {
                    await window.myAPI.mcp.updateServer(mcpServer)
                } else {
                    await updateServer(mcpServer)
                }
                ...
                // 在 Electron 环境下使用 window.myAPI，否则使用 Zustand store
                if (window.electron && window.myAPI?.mcp?.addServer) {
                    await window.myAPI.mcp.addServer(mcpServer)
                } else {
                    await addServer(mcpServer)
                }
    ...
    return (
        ...
        >
            <div className="space-y-6">
                ...
                <FormField label="传输类型" required>
                    <div className="relative">
                        <select
                            value={formData.serverType}
                            onChange={e => setFormData(prev => ({ ...prev, serverType: e.target.value as 'sse' | 'stdio' }))}
                            className={classNames(
                                "w-full px-3 py-2 text-sm rounded-lg",
                                "bg-white dark:bg-gray-900",
                                "border border-gray-300 dark:border-gray-600",
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
                    <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-4">配置测试</h3>
