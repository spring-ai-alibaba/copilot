package com.alibaba.cloud.ai.copilot.model.controller;
...
import com.alibaba.cloud.ai.copilot.model.dto.OpenAiCompatibleRequest;
import com.alibaba.cloud.ai.copilot.model.entity.LlmFactoriesEntity;
import com.alibaba.cloud.ai.copilot.model.entity.ModelConfigEntity;
import com.alibaba.cloud.ai.copilot.model.mapper.LlmFactoriesMapper;
import com.alibaba.cloud.ai.copilot.model.service.ModelConfigService;
import com.alibaba.cloud.ai.copilot.model.provider.ProviderHealthCheckService;
...
public class ModelProviderController {
     ...
     */
    @PostMapping("/health")
    public R<HealthCheckResult> checkProviderHealth(
            @RequestParam String providerCode,
            @RequestParam String apiKey) {
        return R.ok(healthCheckService.checkHealth(providerCode, apiKey));
    }

    /**
     * 检测指定模型健康状态
     * <p>用于用户新增自定义模型名称后的健康检测</p>
     * <p>API Key 从当前登录用户的配置中自动获取</p>
     *
    ...
    public R<HealthCheckResult> checkModelHealth(
            @RequestParam String providerCode,
            @RequestParam String modelName) {
        return R.ok(healthCheckService.checkModelHealth(providerCode, modelName));
    }
