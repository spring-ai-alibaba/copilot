...
public class CopilotAgentFactory {
    ...
    String buildSystemPrompt(
        ...
        return basePrompt + "\n\n" +
                ...
                "信息不足时也不得只向用户追问：先基于当前上下文和明确标注的合理假设完成一份可审批计划，" +
                "再把需要用户决定的事项写入“待确认”章节。只有会让实施方向、数据安全或外部行为无法确定的问题才标为阻塞。\n" +
                "无论是否存在待确认项，探索完成后都必须调用 plan_write，并严格使用以下 Markdown 结构。\n" +
                ...
                "| 文件 | 关键符号 | 操作 | 影响范围 |\n" +
                "|------|----------|------|----------|\n" +
                "关键符号必须填写本次会修改或依赖的类、方法、函数；无法确定时写“无”。\n\n" +
                "## 测试策略\n" +
                "- 新增或更新的测试：\n" +
                "- 手动验证：\n\n" +
                "## 风险点\n" +
                ...
                "- 如果失败，如何恢复：\n\n" +
                "## 待确认（没有则写“无”）\n" +
                "- [阻塞] 仅在必须先得到用户答案才能安全实施时使用；问题；建议：推荐选项\n" +
                "- [非阻塞] 可按推荐方案直接实施、用户稍后仍可调整的事项；问题；建议：推荐选项\n" +
                "不要在正文中只提问而不生成计划；即使有阻塞问题，也必须先写入完整 PLAN.md 并调用 plan_exit。\n\n" +
