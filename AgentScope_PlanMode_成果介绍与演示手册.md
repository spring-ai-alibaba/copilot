...
| CodeGraph 代码理解 | 复杂改动可基于真实代码关系规划 | 会话工作区发现代码后后台自动建索引，执行完成自动增量同步；计划卡会展示关联测试与关键符号影响证据，首次索引未就绪时清晰提示并安全降级 |
...
3. “如何验证”与明确的预期结果；
4. 风险与不修改的边界；
5. CodeGraph 的“代码证据”：哪些测试与本次改动相关，以及关键符号可能波及的代码节点、依赖关系和文件；首次分析若仍在建索引，会显示准备状态，不阻塞审批；
6. 折叠的技术详情：原始计划、Git 状态与改前文件片段；
7. 批准后的 Permission Mode 和中文策略说明。
...
- 将 CodeGraph 索引按仓库身份复用到多个会话，并显示索引时间与同步版本；
