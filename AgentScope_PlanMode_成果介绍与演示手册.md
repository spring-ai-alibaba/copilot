...
flowchart TD
    ...
    G --> H["输入框上方工作面板汇总计划、Git、文件片段、风险和权限"]
...
> 审批不是只看一段模型生成的文字。这里把代码现场一起带到了输入框上方的工作面板：现在工作区是否有未提交修改、计划提到的文件原来是什么样、该计划会采用什么执行权限，用户在批准前都能看见。它不会作为一条历史消息被滚走，用户随时可以收起和回看。
