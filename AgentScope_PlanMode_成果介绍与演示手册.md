...
> 原来的编程 Agent 收到需求后会直接开始改文件，用户很难提前知道它准备改什么、为什么这么改，以及风险在哪里。我参考 Codex 的交互方式，在输入框增加了“计划”模式。开启后，Agent 先只读探索项目，通过实时卡片展示分析和工具调用，再生成结构化计划。计划与 Todo 会集中显示在输入框上方的一张固定工作面板中，不会混入聊天消息。面板会同时给出影响文件、改前代码片段、Git 状态、风险等级和执行权限，支持收起、恢复、批准和带意见驳回。这样既保留了 Agent 自动完成任务的效率，又把关键决策权交还给用户。
...
| 计划与 Todo 统一面板 | 实时看到计划、总进度、当前任务和已完成项 | 固定在输入框上方，正文独立滚动，支持展开和收起 |
...
计划生成后，工作面板会自动展开。先指出它固定在输入框上方，聊天区仍只保留探索过程和回复；然后依次介绍：
...
再手动收起并重新展开一次，说明长计划只在面板正文内部滚动，不会把输入框推出页面，也不会挤占整段聊天历史。
...
- [ ] 至少演示一次驳回
- [ ] 批准后观察统一面板收起、Todo 首次到达后展开、完成后再次收起
